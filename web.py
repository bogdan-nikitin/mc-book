...
import json
from text_to_book import TextToBook, CharError, DefaultConfig
...
will need to specify 3 as the width.
    ...
    if use_default_size:
        ...
        get_config().default_size = default_size
    else:
        get_config().default_size = None
    ...
    for file in character_size_files:
        try:
            ...
            st.error(f'''
During parsing "{file.name}" an error occurred: *{e}*
            ''')
        except Exception as e:
            st.error(f'''
Exception "{e.__class__.__name__}" was raised. Go to {GITHUB_ISSUES_LINK} to
report your problem. Specify the content of the uploaded files.
            ''')
